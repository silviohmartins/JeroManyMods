...
namespace JeroManyMods.Patches.EnvironmentEnjoyer
{
    public class EnvironmentEnjoyerScript : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (_gameWorld == null)
            {
                MainJeroManyMods.Logger.LogError("[EnvironmentEnjoyer] GameWorld not found.");
                Destroy(this);
                return;
            }
        ...
        private async Task ChangeObjects()
        {
            ...
            if (!ReadyToEdit)
            {
                MainJeroManyMods.Logger.LogError("[EnvironmentEnjoyer] Timeout waiting for objects to load.");
                return;
            }

            if (TreeBush != null && TreeBush.Count > 0)
            {
                bool shouldHide = MainJeroManyMods.EnvironmentEnjoyer.Value;
                ...
                foreach (var bush in TreeBush)
                {
                    if (bush != null)
                    {
                        bush.SetActive(!shouldHide);
                        processedCount++;
                    }
                }
                MainJeroManyMods.Logger.LogInfo($"[EnvironmentEnjoyer] Processed {processedCount}/{TreeBush.Count} objects. Hide: {shouldHide}");
            }
            else
            {
                MainJeroManyMods.Logger.LogWarning($"[EnvironmentEnjoyer] No objects found to process. TreeBush is null or empty.");
            }
        }
        ...
        public void GetGameObjects()
        {
            ...
            {
                MainJeroManyMods.Logger.LogError("[EnvironmentEnjoyer] GameWorld or LocationId is null.");
                ReadyToEdit = true; // Marca como pronto para evitar loop infinito
                return;
            }
            
            MainJeroManyMods.Logger.LogInfo($"[EnvironmentEnjoyer] Starting to find objects for location: {_gameWorld.LocationId}");
            ...
            switch (_gameWorld.LocationId.ToLower())
            {
                ...
                case "sandbox_high":
                    ...
                    break;
                default:
                    MainJeroManyMods.Logger.LogWarning($"[EnvironmentEnjoyer] Unknown location: {_gameWorld.LocationId}");
                    break;
            }

            MainJeroManyMods.Logger.LogInfo($"[EnvironmentEnjoyer] Found {TreeBush.Count} objects. Ready to edit: true");
            ReadyToEdit = true;
        }
